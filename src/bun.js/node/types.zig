...
pub const Path = struct {
    ...
    inline fn getPathLenT(comptime T: type, lenSum: usize) usize {
        ...
        return @max(lenSum, if (size.MIN > 0) size.MIN else size.MAX);
    }
    ...
    pub fn PathSize(comptime T: type) type {
        // Specifying 0 for a min size means it will not be used.
        const _MIN: usize = if (T == u16) PATH_MIN_WIDE else 0;
    ...
    pub fn formatJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, pathObject: PathParsed(T)) JSC.JSValue {
        ...
        return switch (getPathLenT(T, lenSum)) {
            size.MIN => if (isWindows) formatWindowsJS_L(T, globalObject, pathObject, size.MIN) else formatPosixJS_L(T, globalObject, pathObject, size.MIN),
            size.MAX => if (isWindows) formatWindowsJS_L(T, globalObject, pathObject, size.MAX) else formatPosixJS_L(T, globalObject, pathObject, size.MAX),
            else => |bufLen| brk: {
    ...
    pub fn joinJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, paths: []const []const T) JSC.JSValue {
        ...
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, lenSum)) {
            size.MIN => if (isWindows) joinWindowsJS_L(T, globalObject, paths, size.MIN) else joinPosixJS_L(T, globalObject, paths, size.MIN),
            size.MAX => if (isWindows) joinWindowsJS_L(T, globalObject, paths, size.MAX) else joinPosixJS_L(T, globalObject, paths, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
    ...
    pub fn normalizeJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, path: []const T) JSC.JSValue {
        ...
        return switch (getPathLenT(T, path.len)) {
            size.MIN => if (isWindows) normalizeWindowsJS_L(T, globalObject, path, size.MIN) else normalizePosixJS_L(T, globalObject, path, size.MIN),
            size.MAX => if (isWindows) normalizeWindowsJS_L(T, globalObject, path, size.MAX) else normalizePosixJS_L(T, globalObject, path, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
    ...
    pub fn relativeJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, from: []const T, to: []const T) JSC.JSValue {
        ...
        return switch (getPathLenT(T, from.len + to.len)) {
            size.MIN => if (isWindows) relativeWindowsJS_L(T, globalObject, from, to, size.MIN) else relativePosixJS_L(T, globalObject, from, to, size.MIN),
            size.MAX => if (isWindows) relativeWindowsJS_L(T, globalObject, from, to, size.MAX) else relativePosixJS_L(T, globalObject, from, to, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
    ...
    pub fn resolveJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, paths: []const []const T) JSC.JSValue {
        ...
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, lenSum)) {
            size.MIN => if (isWindows) resolveWindowsJS_L(T, globalObject, paths, size.MIN) else resolvePosixJS_L(T, globalObject, paths, size.MIN),
            size.MAX => if (isWindows) resolveWindowsJS_L(T, globalObject, paths, size.MAX) else resolvePosixJS_L(T, globalObject, paths, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
    ...
    pub fn toNamespacedPathJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, path: []const T) JSC.JSValue {
        ...
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, path.len)) {
            size.MIN => toNamespacedPathWindowsJS_L(T, globalObject, path, size.MIN),
            size.MAX => toNamespacedPathWindowsJS_L(T, globalObject, path, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
